...
public:
    ...
    /// @brief Enqueues a command list for execution in the immediate context.
    /// @param[in] cmdList the command list to enqueue
    /// @param[in] cbFinish a function to execute when the command list is executed
    void EnqueueCommandList(wil::com_ptr_nothrow<ID3D11CommandList> &&cmdList, std::function<void()> &&cbFinish);
...
private:
    ...
    struct CommandList {
        wil::com_ptr_nothrow<ID3D11CommandList> list;
        std::function<void()> cbExecuted;
    };

    std::mutex m_mtxCmdList{};
    std::vector<CommandList> m_cmdListQueue;
};
