...
struct Direct3D11VDPRenderer::Context {
    ...
    util::Event WaitForFrameEvent{true};
...
Direct3D11VDPRenderer::~Direct3D11VDPRenderer() {
    ...
    m_context->WaitForFrameEvent.Set();
}
...
void Direct3D11VDPRenderer::VDP1SwapFramebuffer() {
    ...
    // This function is invoked before the displayFB bit is flipped.
    // The display framebuffer is ready to be erased (or drawn over).
    // The draw framebuffer has the frame that the VDP1 has just drawn.
    const size_t displayFB = m_state.displayFB ^ 1;
    const size_t drawFB = m_state.displayFB;
...
void Direct3D11VDPRenderer::VDP2BeginFrame() {
    m_context->WaitForFrameEvent.Wait();
    m_context->WaitForFrameEvent.Reset();
...
void Direct3D11VDPRenderer::VDP2EndFrame() {
    ...
    ++m_VDP2FrameCounter;
    m_context->DeviceManager.EnqueueCommandList(std::move(commandList), [this] { m_context->WaitForFrameEvent.Set(); });
