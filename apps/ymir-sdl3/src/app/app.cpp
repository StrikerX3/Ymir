...
void App::RunEmulator() {
    ...
    while (true) {
        ...
        const bool hwRenderSync = !m_context.paused && vdp.GetRenderer().IsHardwareRenderer();
        ...
        if (m_context.emuSpeed.limitSpeed) {
            ...
            if (hwRenderSync) {
                // Use display refresh rate and never go below 48 fps
            ...
            } else if (settings.video.useFullRefreshRateWithVideoSync) {
            ...
            if (screen.videoSync && !hwRenderSync) {
                // Sleep until 1ms before the next frame presentation time, then spin wait for the deadline
            ...
            const int newVSync = !hwRenderSync ? 1 : SDL_RENDERER_VSYNC_DISABLED;
            if (vsync != newVSync) {
                if (SDL_SetRenderVSync(m_graphicsService.GetRenderer(), newVSync)) {
                    devlog::info<grp::base>("VSync disabled");
                    vsync = newVSync;
                } else {
                    devlog::warn<grp::base>("Could not change VSync mode: {}", SDL_GetError());
                }
